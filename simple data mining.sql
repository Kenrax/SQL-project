/*DANE Z https://gist.github.com/Mentrasto/967148a4b41713977839 */
...
SELECT name, MAX(popularity) from dbo.marvels;
SELECT AVG(popularity) from marvels;
SELECT name, MIN(popularity) from marvels;

SELECT dbo.marvels.gender, AVG(popularity) AS avg_popularity FROM marvels
    GROUP BY gender
    HAVING avg_popularity > 5;

SELECT * FROM marvels WHERE hometown in ('USA' , 'EUA');
    
SELECT name, height_m as wzrost, weight_kg as waga, ROUND(weight_kg/(height_m*height_m),2)as BMI,
    CASE 
        WHEN weight_kg/(height_m*height_m) < 18 THEN 'niedowaga'
        WHEN weight_kg/(height_m*height_m) > 18 and weight_kg/(height_m*height_m) < 25 THEN 'waga prawid³owa'
        WHEN weight_kg/(height_m*height_m) > 25 THEN 'nadwaga'
    END as 'Jaka waga ?'
FROM marvels;